...
@Component({
  ...
  template: `
    <app-dialog [open]="open" [title]="entityId ? 'Edit Salesman' : 'New Salesman'" (close)="close.emit()">
...
export class SalesmanFormComponent implements OnChanges {
  ...
  submit() {
    ...
    const obs = this.entityId
      ? this.api.update('salesmen', this.entityId, body)
      : this.api.create('salesmen', body);
