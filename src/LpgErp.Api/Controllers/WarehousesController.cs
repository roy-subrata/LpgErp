...
{
    public WarehousesController(IRepository<Warehouse> repository, IUnitOfWork unitOfWork)
        : base(repository, unitOfWork) { }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] Warehouse warehouse, CancellationToken cancellationToken)
    {
        await Repository.AddAsync(warehouse, cancellationToken);
        await UnitOfWork.SaveChangesAsync(cancellationToken);
        return CreatedAtAction(nameof(GetById), new { id = warehouse.Id }, new { data = warehouse });
    }
